...
- **Search Console Integration:** Optional Google Search Console link (app-wide, one-time OAuth) blends real search performance into the priority ranking; a built-in Connections panel tests AI-provider, GSC and WordPress connectivity — the WordPress row says whether the stored credentials still log in, and whether that account can run the fixes, the configuration audit, or neither.
