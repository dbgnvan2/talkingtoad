...
| **M3** | Google-validated GEO **audit** extensions | ✅ **Done (2026-05-31, orche)** | M3.1 `SCHEMA_VISIBLE_MISMATCH` `345c935` · M3.2 `AI_CONTENT_NOT_IN_TEXT` `96a2a24` · M3.3 `AI_PREVIEW_SUPPRESSED`/`AI_PREVIEW_BLOCKED_AT_BOT` `3d0f119` · M3.4 `AI_NO_VISUAL_COMPANION` `e631756` · M3.5 `AI_MAIN_CONTENT_LOW_RATIO` `e2b53a0` · M3.6 scoring-scope fix `eefa937`. All reviewed; full suite 1562 passed, 0 fail. |
...
| **M6** | Performance-Health Feedback Loop (GSC) | ✅ **Done (2026-06-01, orche)** | Authority Matrix (GSC perf × HealthScore) + Ledger + Refresh Trigger + OAuth ingest. ✅ M6.2 Ledger `a03009b` · ✅ M6.3 Refresh Trigger `d5a965a` · ✅ **M6.1+M6.4 GSC OAuth ingest + endpoints** `5ab994d` (`gsc_client.py` w/ backoff; `/api/gsc/{connect,callback,status,disconnect,ingest,performance}`; PKCE persisted, insecure-transport gated to localhost, siteOwner preferred, opt-in 503, Fernet creds). **OAuth round-trip proven live vs livingsystems.ca via probe.** 27 GSC tests (mocked); full suite 1669 passed. ✅ **M6.4 GSCInsightsPanel** `4bdcbc7` (lazy-loaded additive card, opt-in 503 empty state, V4 explainer; 6 RTL tests; 0 new lint errors). M6 fully complete incl. frontend. |
...
| **M9** | Refactor hotspots | ✅ **Done-enough (2026-06-01)** | M9.1 `issue_checker.py`→`checkers/` (prior). ✅ **M9.4 `wp_heading_fixer.py`→`wp_heading/` package** `84282d8` (1031→47-line facade + sources/edit/widgets/bulk; all imports preserved; suite exactly 1669). Per user: the high-risk big-file splits (`crawl.py`, `Results.jsx`, `sqlite_store.py`) **deferred to post-v3.0 tech debt** — pure maintainability, not v3.0-blocking. |
| **M10** | Frontend infrastructure | ✅ **Done (2026-06-01, orche)** | ✅ **M10.4** auth-helper consolidation `ca7b10a` (3 dup `authHeaders` → canonical `api.js`). ✅ **Frontend test suite repaired** `0ff5ab1` (was 64/148 failing pre-existing → **148/148 pass**, test files only — now a real gate). ✅ **M10.1a** toast infra + 7 small-file migrations `f2c1bae` (ToastContext provider+useToast, success/error/info/confirm; vitest 159/159). ✅ **M10.1b** Results.jsx+ImageAnalysisPanel migrated `af1666d` — **M10.1 toast system COMPLETE (0 raw alert/confirm repo-wide, vitest 159/159)**. ✅ **M10.2 a11y** + **M10.3 code-splitting** `cfbd362` (42 aria-labels, `<main>` on 3 pages, role=dialog on modals, inputMode=url; 5 heavy modals lazy-loaded; vitest 159/159). ✅ **M10.5** coverage `d69a42e` (ErrorBoundary, FixBrokenLinkPanel, AIReadinessPanel, GEOReportPanel tests; vitest **176/176**). **M10 COMPLETE.** |
| **M11** | Docs sync & v3.0 release | ✅ **RELEASED v3.0 (2026-06-01)** | ✅ **M11-docs** `0faa678`: `api.md` + new shipped endpoints; new `docs/ai-readiness.md`, `docs/security-model.md`, `docs/ai-routing.md` (status:draft, parked features marked parked); README index. No version bump (still 2.6.0). ✅ **M11-release** `d4c433d`: version→3.0.0 (main.py/utility.py/CLAUDE.md), v3.0 feature note, 142-code count. Smoke: python 1669 + frontend 176 + build all green. **Tag `v3.0` pushed.** |
...
| 3.4 Authoritative Entity Schema Factory | **GA4** | ✅ **Done (2026-05-31, orche)** | `api/services/geo_schema_factory.py` + `POST /api/geo/entity-schema` + GeoConfig `entity_wikipedia_url` (round-trips, back-compat) + entity card + V4 explainer; 28 tests incl. sameAs-absent adversarial; reviewed, no nits. Commit `da0a52c`. **Workstream GA complete.** |
