...
| **M6** | Performance-Health Feedback Loop (GSC) | 🟡 **Mocked half done; OAuth half blocked** | Authority Matrix (GSC perf × HealthScore) + Ledger + Refresh Trigger. ✅ **M6.2 Performance Ledger** `a03009b` (model + store upsert keyed on (url,period)). ✅ **M6.3 Refresh Trigger** `d5a965a` (staleness >180d / decay >20% / Vulnerable-Star / Hidden-Gem; deterministic `today`; 22 tests). Full suite 1642 passed. **⏸️ 6.1 live OAuth ingest + 6.4 panel blocked on Google creds** (client ID received; local redirect `http://localhost:8000/api/gsc/callback`). Spec: `docs/pending/2026-06-01_m6_performance_health_feedback_loop.md` |
