...
| **M6** | Performance-Health Feedback Loop (GSC) | 🟡 **In progress** | Reframed as Authority Matrix (GSC perf × HealthScore) + Performance Ledger + Refresh Trigger. **M6.2 Performance Ledger done** `a03009b` (model + store upsert keyed on (url,period); 9 tests; full suite 1620). M6.3 (refresh trigger, mocked) next. **6.1 live OAuth + 6.4 panel blocked on user's Google credentials.** Spec: `docs/pending/2026-06-01_m6_performance_health_feedback_loop.md` |
