...
| **M10** | Frontend infrastructure | 🟡 **In progress** | ✅ **M10.4** auth-helper consolidation `ca7b10a` (3 dup `authHeaders` → canonical `api.js`). ✅ **Frontend test suite repaired** `0ff5ab1` (was 64/148 failing pre-existing → **148/148 pass**, test files only — now a real gate). Remaining: M10.1 toasts, M10.2 a11y, M10.3 code-splitting, M10.5 coverage. |
