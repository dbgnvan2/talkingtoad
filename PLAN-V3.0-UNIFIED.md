...
| **M10** | Frontend infrastructure | 🟡 **In progress** | ✅ **M10.4** auth-helper consolidation `ca7b10a` (3 dup `authHeaders` → canonical `api.js`). ✅ **Frontend test suite repaired** `0ff5ab1` (was 64/148 failing pre-existing → **148/148 pass**, test files only — now a real gate). ✅ **M10.1a** toast infra + 7 small-file migrations `f2c1bae` (ToastContext provider+useToast, success/error/info/confirm; vitest 159/159). ✅ **M10.1b** Results.jsx+ImageAnalysisPanel migrated `af1666d` — **M10.1 toast system COMPLETE (0 raw alert/confirm repo-wide, vitest 159/159)**. ✅ **M10.2 a11y** + **M10.3 code-splitting** `cfbd362` (42 aria-labels, `<main>` on 3 pages, role=dialog on modals, inputMode=url; 5 heavy modals lazy-loaded; vitest 159/159). Remaining: M10.5 coverage gaps. |
