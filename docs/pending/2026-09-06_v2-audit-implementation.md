...
>
> This is pre-existing and not caused by the new codes, so **decide, do not drift**: either (a) accept it and scope acceptance criterion 17 to the drawer for those three, or (b) make "carry an evidence tier into the PDF and Excel for every category" an explicit deliverable with its own criterion. What is not allowed is marking criterion 17 done on the strength of criterion 12 — `authority.yaml` reaches neither surface.
>
...
17. The evidence-tier decision in §1.2 is recorded, and this criterion is graded against whichever branch was chosen — **not** against criterion 12, which concerns a file that does not reach the product. Under branch (a) all five codes carry a drawer tier and only codes 3 and 4 carry one in the PDF/Excel; under branch (b) all five carry it in both. Either way, a measurement that could not be made renders *not checked*, never clean (repo P2 rule).
