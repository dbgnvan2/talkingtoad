...
> - **In the PDF and Excel** — `Issue.confidence_label`, fed by `_AI_READINESS_CONFIDENCE`, rendered as `Evidence: …` by `report_generator.py` and `excel_generator.py`. That dict is `ai_readiness`-only, enforced in **both** directions by `test_confidence_entries_only_for_ai_readiness_category`. Measured: `make_issue("IMG_ALT_MISSING", …).confidence_label` is `None`; `ENTITY_NAP_INCOMPLETE`'s is `"Established"`.
>
...
>
> How, and what not to do: **do not** add the new codes to `_AI_READINESS_CONFIDENCE` — `test_confidence_entries_only_for_ai_readiness_category` holds that dict to one category in both directions and will turn red. The tier already exists for all 170 codes in `issueHelp.json`'s `confidence` field, which is what the drawer renders; the export needs that same value from **one** source, never a second copy. Two routes, the choice being the implementer's on the evidence:
...
`_check_entity_values` picks `rep` as the page matching the start URL whenever one exists, and falls back to the **first page in list order** carrying an entity node only when no page matches (the code takes `candidates[0]`; "shallowest" is its docstring, true only if `pages` happens to be depth-ordered). So on a site whose `Organization`/`LocalBusiness` node lives on `/about` but not on the homepage, the representative page yields no entity nodes and the check **silently never fires** — no finding, no disclosure, indistinguishable from a pass (P3/P31).
