...
> **Owner decisions (2026-09-06) — the four open questions r1–r4 left are now answered, and none is the implementer's to reopen.**
> 1. **`LOW_INBOUND_LINKS` is not gated on `archives_skipped`.** Count everything: search engines see the archives whether this tool does or not, so a page linked only from archives is genuinely weakly linked, and the fix is for the site to hide those archives or make them worth indexing. Flag both zero inbound (`ORPHAN_PAGE`) and one (`LOW_INBOUND_LINKS`). The `link_graph_complete` gate is untouched — that one suppresses on *inability to see*, which is a different thing. (§1.5)
> 2. **`IMG_MISSING_DIMENSIONS` counts decorative images**, and reports `decorative_count` beside `count`. Suppressing on `is_decorative` would drop a full-width `alt=""` hero — the image most likely to be shifting the page — to spare tracking pixels. Disclose the composition; do not suppress the class. (§1.1)
> 3. **The homepage is not special.** Codes 3 and 4 search every crawled page for entity nodes rather than only the representative page; emission stays site-scoped and once, at the page that carries the node. (§1.4)
> 4. **The evidence tier goes into the PDF and Excel, for every category**, not only `ai_readiness`. One value, one source — a third copy of this label is the defect its parity test already exists to catch. (§1.2)
>
> Each is written into the section it governs and into an acceptance criterion, with a test specified that fails if the opposite is built.
...
2. **Source of truth is `api/crawler/checkers/registry.py`.** A code must be registered in all of:
   ...
   - `_AI_READINESS_CONFIDENCE` — **required for every `ai_readiness` code** (`Established` / `Reasonable proxy` / `Heuristic`), enforced by `tests/test_architecture_constraints.py::test_every_ai_readiness_code_has_confidence_label`. It covers 75 of 170 codes, so it is easy to miss; both new entity codes need an entry, and it must agree with the `_CALIBRATION` confidence. **The label has a seventh home with wider coverage:** `frontend/src/data/issueHelp.json` carries its own `confidence` field for **all 170** codes — it, not this dict, is what the help drawer renders. `tests/test_confidence_help_parity.py` fails when the two disagree on an `ai_readiness` code, but is silent when the field is simply **absent**, so omitting it ships a drawer with no tier and nothing goes red. See §1.2.
...
*Decided (owner, 2026-09-06): **count every image, and report the decorative share separately**.* `parser.py` already computes `is_decorative` per image (`role="presentation"`, `aria-hidden="true"`, `alt=""`, sub-32px). It does **not** suppress this code.

  The reasoning matters, because "decorative" looks like the obvious suppressor and is the wrong one. `is_decorative` is the markup's claim about an image's *meaning*; what predicts a layout shift is its *size* — and an image with no declared dimensions is by definition one whose size we do not know, so size cannot filter it. The list includes `alt=""`, and a full-width hero with `alt=""` is among the images most likely to be shifting the page. Suppressing on that signal would drop the worst case to spare the harmless ones.

  So: emit for every `<img>` that does not declare both dimensions, and make the finding say what it is made of —
  `extra={"count": N, "decorative_count": D, "examples": [...][:5]}` — with the description reading in the shape of *"40 images do not declare both dimensions (12 marked decorative)"*. The operator sees a site with thirty tracking pixels for what it is, without the tool having guessed on their behalf. This is the same principle as the `LOW_INBOUND_LINKS` decision in §1.5: count it, disclose the shape, let the operator decide.

  The failure this is steering around is on record: `IMG_ALT_MISSING` produced 156 findings on a site whose alt text was correct, every one an `alt=""` decorative image, guarded by a test labelled "adversarial" that pinned the wrong answer because its expected value came from the implementation rather than the standard (P32). The lesson taken here is **disclose the composition**, not **suppress the class** — suppression is what would have hidden the hero image.
...
*Adversarial test:* an `<img>` with `width` set but `height` missing (or vice-versa) **must** count; an `<img>` with both present must not; an `<img>` with `width="" height=""` must count; an `<img>` with `width="100px"` and `height="60"` must not (both parse); a decorative `<img>` (`alt=""`, `role="presentation"`) with no dimensions **must** count **and** must raise `decorative_count`; a fixture mixing 3 decorative and 2 ordinary images must report `count=5, decorative_count=3` — a test asserting only `count` would pass against an implementation that silently suppressed the class, so assert both.
...
> **The rendered evidence tier comes from neither the `_IssueSpec` nor `authority.yaml`. It has two surfaces with different coverage, and only one of them covers all five new codes.**
> - **On screen** — `frontend/src/data/issueHelp.json`'s own `confidence` field, rendered by `IssueHelpPanel.jsx` (`data-testid="help-confidence"`) and `AIReadinessPanel.jsx`. It is populated for **all 170** codes, so every new code gets a drawer tier — provided the field is written. `tests/test_confidence_help_parity.py` compares it to `_AI_READINESS_CONFIDENCE`, but its reader is `{code: e["confidence"] for … if e.get("confidence")}` — an **absent** field is silently skipped, so omitting it ships a tier-less drawer and nothing goes red. Write it, and check it by hand.
> - **In the PDF and Excel** — `Issue.confidence_label`, fed by `_AI_READINESS_CONFIDENCE`, rendered as `Evidence: …` by `report_generator.py` and `excel_generator.py`. That dict is `ai_readiness`-only, enforced in both directions by a pair of tests in `tests/test_architecture_constraints.py` (see §1.2). Measured: `make_issue("IMG_ALT_MISSING", …).confidence_label` is `None`; `ENTITY_NAP_INCOMPLETE`'s is `"Established"`.
>
> So codes 3 and 4 carry a tier everywhere; `IMG_MISSING_DIMENSIONS`, `LOW_INBOUND_LINKS` and — the awkward one — `FORM_FIELD_NO_LABEL`, the single new code resting on a normative standard, carry one **on screen but not in the exported report**. Adding them to `_AI_READINESS_CONFIDENCE` is not the workaround: that test turns red in the other direction.
>
> **Decided (owner, 2026-09-06): put the tier in the export.** Carrying an evidence tier into the PDF and Excel **for every category** is an explicit deliverable of this spec, not a deferred nicety. A WCAG conformance failure that exports without its basis is the report telling a nonprofit less than the screen already told them.
>
> How, and what not to do: **do not** add the new codes to `_AI_READINESS_CONFIDENCE`. Two tests in `tests/test_architecture_constraints.py` hold that dict to one category, one per direction — `test_every_ai_readiness_code_has_confidence_label` (no `ai_readiness` code may be missing) and `test_confidence_entries_only_for_ai_readiness_category` (no other category may be present). The second turns red on a `performance` or `crawlability` entry. The tier already exists for all 170 codes in `issueHelp.json`'s `confidence` field, which is what the drawer renders; the export needs that same value from **one** source, never a second copy. Two routes, the choice being the implementer's on the evidence:
> - promote the per-code confidence into the backend as a registry-derived value the serialiser attaches to every `Issue`, with `_AI_READINESS_CONFIDENCE` becoming a view of it rather than a separate list; or
> - have `report_generator.py` and `excel_generator.py` read the generated Python help copy (`api/services/issue_help_data.py`), which already carries the field.
>
> One thing to fix while you are in there: `report_generator.py`'s Evidence line is *already* category-agnostic — it gates only on `if first.confidence_label:` — but the comment above it still reads `# Confidence evidence line (AI Readiness issues)`, describing a restriction the code does not have. It will read as intent and stop someone. Correct it in the same change. The Excel side is genuinely restricted: `excel_generator.py` writes a Confidence column only on the AI Readiness sheet.
>
> Whichever is chosen, the constraint is the same: **one value, one source.** This label has already drifted across its homes once — `tests/test_confidence_help_parity.py` exists because the drawer went on telling users a vendor had confirmed `ENTITY_HOURS_DEFAULT` after the API had downgraded it. A third copy in the PDF would be that defect a third time. Extend the parity test to cover the export surface in the same change.
>
> *(An earlier draft of this paragraph said those three codes render no tier at all, anywhere. That was wrong — it read the PDF path and assumed the drawer shared it. The drawer has its own field, populated for every code.)*
...
*Adversarial test:* `legalName` present → no flag (whatever its casing); `legalName` absent → flag; `name` absent → no flag (nothing to compare); three pages carrying the same node → exactly one finding; the node on `/about` and not on the homepage → the finding still fires, per the representative-page section below.
...
*Adversarial test:* all configured fields present → no flag; only `geo` missing → flag with `["geo"]`; `geo` present with `latitude` only → flag; a plain `Organization` (not a premises type) → no flag; three pages carrying the node → exactly one finding; the node on `/about` and not on the homepage → the finding still fires, per the representative-page section below.

### Representative-page scope — codes 3 and 4 (decided: find the node wherever it lives)

`_check_entity_values` picks `rep` as the page matching the start URL whenever one exists, and falls back to the **first page in list order** carrying an entity node only when no page matches (the code takes `candidates[0]`; "shallowest" appears only in the inline comment above it (`cross_page.py:679`) and is true only if `pages` happens to be depth-ordered). So on a site whose `Organization`/`LocalBusiness` node lives on `/about` but not on the homepage, the representative page yields no entity nodes and the check **silently never fires** — no finding, no disclosure, indistinguishable from a pass (P3/P31).

**Decided (owner, 2026-09-06): the homepage is not special.** Neither "not applicable" nor "not checked" is the right answer, because both accept a scope that was never intentional. Codes 3 and 4 must **search every crawled page for entity nodes** and evaluate the ones they find, wherever they live.

Implement it as:
- **Detection ranges over all pages** — collect entity nodes from every page in `pages`, not from `rep` alone.
- **Emission stays site-scoped and once.** That half of R5.1 is unchanged. Emit at the page that actually **carries** the node (preferring the start URL's page when it carries one) so the finding points somewhere real, and put the carrying URL in `extra` (`{"found_on": …}`) so the operator can go straight to it.
- **Deduplicate by node identity, not by page.** The same Yoast `Organization` node repeated across 272 pages is one finding; two genuinely different nodes (an `Organization` and a separate `LocalBusiness`) are two.

This widens `_check_entity_values`'s behaviour, which the existing `ENTITY_*` codes also sit inside. **Do not silently widen those along with it** (global rule 10). If the same change would alter what `ENTITY_NAP_INCOMPLETE` or `ENTITY_HOURS_DEFAULT` report on a site whose node is off the homepage, that is a real improvement *and* a change to existing findings and to the health score: surface it in the plan as an adjacent issue with a before/after count from a real crawl, and let the owner take it as its own decision.

*Adversarial test:* the entity node on `/about` only → the finding fires and `extra["found_on"]` is `/about`; the node on the homepage **and** `/about` → exactly one finding; two structurally different nodes on two pages → two findings; no entity node anywhere → no finding **and** no claim of a pass.
...
- **Do not rename `orphan_detection`.** It is a persisted SQLite column (`api/services/sqlite_store.py`), read back for stored jobs. Renaming it blanks the disclosure on every historical job unless a migration ships with it — and `cafdf6e`, the commit this spec set landed directly on top of, is a migration incident. Extend the payload; leave the key alone.
...
*Decided (owner, 2026-09-06): **no `archives_skipped` gate**.* `LOW_INBOUND_LINKS` behaves exactly like `ORPHAN_PAGE` here — `skip_wp_archives` stays **disclosed, not gated**. The reasoning is worth recording, because it will otherwise read as noise-tolerance: search-engine crawlers see the archive pages whether or not this tool does, so a page whose only inbound links come from archives really is weakly linked from the reader's point of view. The right response is for the site to suppress those archives or make them worth indexing — that is a finding, not a false positive. Report it and let the operator decide.

  What this does **not** change: the `link_graph_complete` gate above stands unaltered. A partial scan, a `max_pages` truncation and single-page mode still suppress the check entirely, because those leave the tool unable to see links that exist. `skip_wp_archives` is different in kind — the pages are excluded by a setting the operator chose and can see, and the disclosure names it.

  Adjacent, not fixed (global rule 10): whether `skip_wp_archives` should still default to **on** is a live question this decision implies but does not settle. Out of scope here; recorded in `TODO.md` rather than changed in passing.
...
8. `LOW_INBOUND_LINKS` has **no** `archives_skipped` gate (owner decision, §1.5) — archives are disclosed exactly as they are for `ORPHAN_PAGE`. A test asserts the check still fires on a crawl with `skip_wp_archives=True`, so a later tidy-up cannot quietly add the gate.
...
10. `IMG_MISSING_DIMENSIONS` counts decorative images and reports `decorative_count` alongside `count` (owner decision, §1.1); the mixed-fixture test asserts **both** numbers, so an implementation that suppressed the class cannot pass.
...
12. `api/crawler/checkers/data/authority.yaml` has an entry for each of the five new codes, well-formed per §0.4; `FORM_FIELD_NO_LABEL`'s is `basis: citation` / `source_type: standard` citing WCAG 1.3.1; `scripts/verify_authority_urls.py` has been run and `url_verification.yaml` updated; `tests/test_authority.py` is green. `_AI_READINESS_CONFIDENCE` has an entry for both `ai_readiness` codes, **and all five codes have a `confidence` value in `issueHelp.json`** — that field is what the help drawer renders, for every category, and its parity test is silent on an absent field, so this one is checked by hand (§0.2, §1.2).
...
17. Every finding carries its evidence tier in the **PDF and Excel as well as the drawer**, for every category — not only `ai_readiness` (owner decision, §1.2). The value comes from one source, not a third copy, and `tests/test_confidence_help_parity.py` is extended to cover the export surface. Grade this on its own evidence, **not** on criterion 12: `authority.yaml` reaches neither surface. A measurement that could not be made renders *not checked*, never clean (repo P2 rule).
18. `ORG_LEGAL_NAME_MISSING` and `LOCAL_BUSINESS_FIELD_INCOMPLETE` detect over **all** crawled pages, not the representative page alone (owner decision, §1.4); a fixture carrying the entity node only on `/about` produces the finding, with `extra["found_on"]` naming that page. Any resulting change to the existing `ENTITY_*` codes is surfaced with a before/after count, not absorbed.
19. `SCORING_MODEL_VERSION` = `2026-09-06-r7` and `ISSUE_EMISSION_VERSION` = `2026-09-06-e2` are bumped; `docs/functional-specification.md` and `docs/thresholds.md` are folded; `PLAN-V4.0.md` tallies any shipped V4 explainer; `git push origin main` after each item.
20. Full suite green on **both** interpreters (3.11 and 3.14) via `./venv/bin/python -m pytest tests/ -q`.
