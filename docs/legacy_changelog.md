...
# Legacy Changelog & Feature Descriptions

> **Why this file exists:** These descriptive and historical sections used to live in `CLAUDE.md`. They've been evicted as part of the v2.3 M0.10 audit so `CLAUDE.md` can stay a lean, high-density rulebook. Nothing here is authoritative — for current behaviour see `docs/functional-specification.md`, `docs/issue-codes.md`, `docs/thresholds.md`, and the per-feature specs under `docs/specs/`.

Use this file when you need to know **why** something landed historically. Do not edit to track current behaviour — update the canonical docs instead.

---

## Key Features (snapshot, v1.9.1)

1. **Crawler:** Async engine with robots.txt/sitemap support and 50+ SEO issue checks (now 131 — see `docs/issue-codes.md`).
...
## v1.9.4 — Cross-Site Safety, Domain Display, Frontend Quality, Reports

### Security: WordPress Domain Validation
...
## v1.9.5 — New Issue Codes, Crawl Comparison, AI Executive Summary, UI
...
- **Model:** `gemini-1.5-flash` (v1 stable endpoint) — note: some newer paths use `gemini-2.0-flash`; check call sites in `api/services/ai_analyzer.py` for current model strings.
...
### AI-Readiness Issue Codes (legacy snapshot)
...
## Issue Categories (legacy snapshot)
...
**Scoring (historical formula):** Each issue has `impact` (1-10) and `effort` (1-5). Priority = `(impact × 10) − (effort × 2)`. Health Score: `max(0, 100 − Σ issue impacts)` across all issues. For the current canonical formulas see `docs/thresholds.md`.

---

## Source-of-truth pointers (current, not legacy)

If you came here looking for the live behaviour:

- **Issue codes:** `docs/issue-codes.md` (auto-generated from `_CATALOGUE`)
- **Thresholds & scoring formulas:** `docs/thresholds.md`
- **System architecture:** `docs/architecture.md`
- **API reference:** `docs/api.md`
- **Per-feature specs:** `docs/specs/`
- **Functional specification (read-only master):** `docs/functional-specification.md`
