    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="min-h-[400px]">
        ...
        {!activeSeverity && activeTab === TAB_ORPHAN_IMAGES && (
          <OrphanedImagesPanel jobId={jobId} domain={domain} />
        )}
        ...
        <div className="mt-3 p-4 bg-orange-50 border border-orange-200 rounded-lg">
          ...
          <div className="space-y-3">
            {(iss.extra.empty_anchors || iss.extra.empty_anchor_hrefs.map(h => ({ href: h }))).map((anchor, idx) => {
              ...
              if (fix.fixed) {
                return (
                  <div key={href} className="p-3 bg-green-50 rounded-lg border border-green-200 flex items-center gap-2">
              ...
              return (
                <div key={href} className="p-3 bg-white rounded-lg border border-orange-100">
        ...
        <div className="mt-3 p-4 bg-amber-50 border border-amber-200 rounded-lg">
          ...
          <ul className="space-y-1">
            {iss.extra.duplicate_urls.map((url, idx) => (
              <li key={url} className="flex items-center gap-2 p-2 bg-white rounded-lg border border-amber-100">
            ...
            <div className="space-y-2">
              <div>
                ...
                <ul className="list-disc list-inside text-sm text-gray-700">
                  {(Array.isArray(recommendations.h2.current) ? recommendations.h2.current : [recommendations.h2.current]).map((h2) => (
                    <li key={h2}>{h2}</li>
                  ))}
                </ul>
              </div>
              <div>
                ...
                <ul className="list-disc list-inside text-sm text-gray-800 font-medium">
                  {(Array.isArray(recommendations.h2.suggested) ? recommendations.h2.suggested : [recommendations.h2.suggested]).map((h2) => (
                    <li key={h2}>{h2}</li>
                  ))}
                </ul>
              </div>
      ...
      {!isArray ? (
        ...
        <div className="space-y-4">
          {recs.map((rec, idx) => {
            ...
            return (
              <div key={`${rec.category}-${rec.priority}-${idx}`} className="bg-white rounded-lg p-4">
