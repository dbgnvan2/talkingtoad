...
- [ ] **Nested card containers (E6):** `_find_stacked_links` stops at the FIRST card-like ancestor and de-duplicates by container, so a card nested inside another card-like container is decided by walk order rather than by which is really the card. The 2026-09-01 sweep worked around the two known cases (`wp-block-post-*`, `elementor-post__*`) by listing the inner elements in `non_card_classes`, which is whack-a-mole. The real fix is to choose between nested candidates deliberately — prefer the innermost that still bounds a single item — and needs its own micro-spec.
