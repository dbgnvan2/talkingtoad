...
class TestRedisParity:
    """M6 — the Redis store is the production data layer; conftest's `store` is
    SQLite, so nothing here was exercised. Asserts the field survives the
    serialise/parse round trip without needing a live Redis."""

    @staticmethod
    def _redis_store():
        """A RedisJobStore instance with no connection — the serialise/parse
        helpers are pure methods, so the round trip is testable without Redis."""
        from api.services.redis_store import RedisJobStore

        return RedisJobStore.__new__(RedisJobStore)

    def test_o5_round_trips_through_the_real_redis_serialisers(self):
        """Behavioural, not a source grep: writes a job with the real
        _job_to_mapping and reads it back with the real _mapping_to_job."""
        store = self._redis_store()
        payload = {"status": "skipped_partial_scan", "pages_analysed": 37,
                   "pages_out_of_scope": 235, "archives_skipped": True}
        job = CrawlJob(job_id=str(uuid4()), target_url="https://example.com",
                       started_at=datetime.now(timezone.utc),
                       orphan_detection=payload)
        assert store._mapping_to_job(store._job_to_mapping(job)).orphan_detection == payload

    def test_o5_legacy_hash_without_the_key_reads_back_none(self):
        """A hash written before this field existed must not read as complete."""
        store = self._redis_store()
        job = CrawlJob(job_id=str(uuid4()), target_url="https://example.com",
                       started_at=datetime.now(timezone.utc))
        mapping = store._job_to_mapping(job)
        del mapping["orphan_detection"]          # legacy hash: key absent entirely
        assert store._mapping_to_job(mapping).orphan_detection is None

    def test_o5_field_is_in_the_redis_update_allowlist(self):
        """update_job raises on unknown fields, so the router's write would fail
        at runtime if the allowlist were missed. Parsed from the AST, not
        grepped: a bare substring would also match a comment (P19 corollary)."""
        import ast as _ast
        from api.services import redis_store as rs

        tree = _ast.parse(textwrap.dedent(_inspect.getsource(rs.RedisJobStore.update_job)))
        allowed = {
            elt.value
            for node in _ast.walk(tree)
            if isinstance(node, _ast.Assign)
            and any(getattr(tgt, "id", "") == "_ALLOWED" for tgt in node.targets)
            for elt in getattr(node.value, "elts", [])
            if isinstance(elt, _ast.Constant) and isinstance(elt.value, str)
        }
        assert allowed, "could not locate the _ALLOWED set — update this test"
        assert "orphan_detection" in allowed
