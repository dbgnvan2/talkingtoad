...
> | **Phase 4 — four user-value features (2026-09-02)** | *No new codes* — striking-distance pages (with `basis` saying why an empty list is empty, and `null` rather than an invented query), a compare card that strikes an incomparable delta through with the reason, re-check-all in place (explicitly distinguished from Rescan in the button's own title), a WordPress audit that can finally be run from the app, and Fix Focus's third state `not_checked`. The V4 thread: every one of these could have shown a clean number for something it did not measure, and each says so instead. | ✅ (each surface explains its scope inline; no per-code explainer involved) |
...
> | **ND1–ND3 — a duplicate names its partners (2026-09-02)** | *No new codes* — a V4 **"how it can mislead"** correction on an existing one, and the second finding this month whose evidence was rendered under the name of the variable holding it. `NEAR_DUPLICATE_BODY` appeared on one page per group, and its evidence line read "Members:" followed by a list that included *the page you were looking at* — a set where the reader needed a relationship, which taken literally says the page duplicates itself. On livingsystems.ca it often literally did: the bare origin and its slashed twin normalised to two URLs, so 19 of the 37 near-duplicate rows in the dev database were the home page against itself. Now every page in a group carries the finding and names the others under **"Near-identical to"**, and the origin is the root. The explainer's caveat says what the reader will otherwise mis-count: four rows for a group of four pages is four pages sharing one problem, not four problems — and it costs the health score once. | ✅ (`how_it_can_mislead` updated; the row-count/score distinction is stated where a reader would otherwise infer it wrongly) |
> | **WA1–WA5 — the WordPress audit, and a way to see the connection (2026-09-02)** | *No new codes* — a V4 honesty surface on an **integration**. The configuration audit had shipped a day earlier asking for REST routes that do not exist; every call 404ed and no user ever saw a report. Its capability probe accepted a 404 as a pass, so the audit believed it had verified an account it had not, and Site Health parsed every real WordPress payload to nothing. The V4-shaped part is the new **WordPress row in Connections**: it distinguishes *not configured* from *credentials rejected* from *logged in but under-privileged* — an editor can run every fix and still cannot list plugins, and one green tick for both would send the operator to a button that 403s. And the login error now names the URL it posted to, whether it was redirected, and the four causes it cannot tell apart, instead of asserting "check your password" at someone whose password was correct. | ✅ (integration-level explainer, always rendered; each state carries its own remedy) |
> | **SV1–SV2 — a finding that was wrong every time it fired (2026-09-03)** | *No new codes* — a V4 **"how it can mislead"** case caught in the wild on an existing one, and the most expensive kind: `SCHEMA_VISIBLE_MISMATCH` carries impact 6 and cites Google, so a false positive both deducts heavily and tells a nonprofit they may be penalised. It compared a JSON-LD value that still held `&#8217;` against visible text where the entity had already been decoded, and reported text identical to a reader as missing from the page. 98 of 98 headline findings were that artifact. The V4-shaped part is what the report now shows: the value is decoded before display, so the operator sees the words they would see in the WordPress editor instead of `Emotions don&#8217;t take a vacation` — a finding you cannot match against your own page is one you cannot act on or disagree with. | ✅ (the code's own six-part explainer was already shipped; this fixes what the evidence line renders) |
> | **P7.1 — panel explainers (2026-09-04)** | *No new codes* — the V4 five-part shape applied to **tools** rather than findings, which is the half of the education layer that was still missing. Nine panels now carry what it is · why it's useful · good vs bad · how it can mislead · how to use, from one component and one data file. The four that had nothing are the ones that CHANGE a nonprofit's site, and their caveats are the point: a FAQ-schema refusal usually means JavaScript-only answers Google cannot see either; GEO settings are unverified inputs that reach every generated alt text and EXIF field; the image score measures metadata and weight, not whether the picture is any good; and batch optimisation uploads new files while your pages keep serving the old ones until you swap them by hand. That last one is "how it can mislead" at its most literal — the tool finishes successfully and the site is unchanged. | ✅ (9 panels; substance enforced by a test that is itself proved able to reject a vapid caveat) |
