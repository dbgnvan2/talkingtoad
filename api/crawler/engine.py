...
# P8.3: imported as a MODULE so the default_factory reads the constant at call
# time. `from ... import _DEFAULT_PAGE_SIZE_LIMIT_KB` would bind the value once
# at import and re-create the copy this change removes.
from api.crawler.checkers import registry as _registry
...
class CrawlSettings:
    ...
    # Page HTML size threshold in KB — pages larger than this are flagged as
    # PAGE_SIZE_LARGE. Per-job-configurable by design (docs/thresholds.md), but
    # its DEFAULT is the registry's, not a second copy of 300 (P8.3). A plain
    # `= _DEFAULT_PAGE_SIZE_LIMIT_KB` would still bake in a copy at class-
    # definition time, so the factory resolves it per instance and a test proves
    # the link is live by patching the constant.
    page_size_limit_kb: int = field(
        default_factory=lambda: _registry._DEFAULT_PAGE_SIZE_LIMIT_KB)
